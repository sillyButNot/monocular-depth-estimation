...
def vertical_pooling(x, out_height, out_width):
    B, C, H, W = x.shape
    x = x.permute(0, 2, 3, 1)
    x = torch.reshape(x, (B, H, W, C // (out_height*out_width), out_height, out_width))
...
class bts(nn.Module):
    def __init__(self, params, feat_out_channels, num_features=512):
        ...
        self.get_depth  = torch.nn.Sequential(nn.Conv2d(num_features // 16, 1, 3, 1, 1, bias=False),
                                              nn.Sigmoid())
        ##수정
        self.vertical_height, self.vertical_width = num_features // 8, num_features // 16
    def forward(self, features, focal):
        ...
        ## 수정
        vertical_out = vertical_pooling(skip3, self.vertical_height, self.vertical_width)
        concat1 = torch.cat([upconv1, reduc1x1, depth_2x2_scaled, depth_4x4_scaled, depth_8x8_scaled,vertical_out], dim=1)
