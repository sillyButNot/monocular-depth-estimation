...
def main_worker(gpu, ngpus_per_node, args):
    ...
    var_sum = [var.detach().cpu().numpy().sum() for var in model.parameters() if var.requires_grad]
    var_cnt = len(var_sum)
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            depth_class = torch.autograd.Variable(sample_batched['class_gt'].cuda(args.gpu, non_blocking=True))
            lpg8x8, lpg4x4, lpg2x2, reduc1x1, depth_est, depth_class_est = model(image, focal)
            ...
            if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                ...
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                    ...
                    for i in range(num_log_images):
                        ...
                        writer.add_image('reduc1x1/image/{}'.format(i), normalize_result(1 / reduc1x1[i, :, :, :].data),
                                         global_step)
                        writer.add_image('lpg2x2/image/{}'.format(i), normalize_result(1 / lpg2x2[i, :, :, :].data),
                                         global_step)
                        writer.add_image('lpg4x4/image/{}'.format(i), normalize_result(1 / lpg4x4[i, :, :, :].data),
                                         global_step)
                        writer.add_image('lpg8x8/image/{}'.format(i), normalize_result(1 / lpg8x8[i, :, :, :].data),
                                         global_step)
            ...
            if global_step and global_step % steps_per_epoch == 0:
                if not args.multiprocessing_distributed or (
                        args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                    checkpoint = {'global_step': global_step,
                                  'model': model.state_dict(),
                                  'optimizer': optimizer.state_dict()}
                    torch.save(checkpoint, args.log_directory + '/' + args.model_name + '/model-epoch-{}'.format(epoch))
